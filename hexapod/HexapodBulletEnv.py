...
class HexapodBulletEnv(Env):
    def __init__(self, client, time_step = 0.05, frameskip = 12, render = False,  max_velocity = 59*2*math.pi/60, max_torque = 1.50041745): # Values of these parameters were assigned to match the specifications of AX-12A servo motor
        ...
        self.Hexapod = p.loadURDF("/home/qa/Downloads/Hexapod/Hexapod/Hexapod.urdf", basePosition = INIT_POSITION, flags = flags) # The URDF file of the robot is attached with the folder
        self.Plane = p.loadURDF("/home/qa/Downloads/Hexapod/Hexapod/Plane.urdf")
    ...
    def _get_reward(self):
        ...
        reward = w1 * (math.sqrt(TARGET_POSITION[0] ** 2 + TARGET_POSITION[1] ** 2) - math.sqrt((TARGET_POSITION[0] - position[0]) ** 2 + (position[1] - TARGET_POSITION[1]) ** 2)) - w2 * (abs(position[2] - TARGET_POSITION[2])) - w3 * consumption - w4 * (abs(ori[0]) + abs(ori[1]))
        reward *= 0.01
        return reward
